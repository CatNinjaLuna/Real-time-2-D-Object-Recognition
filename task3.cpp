...
Date: Oct/28/2024
File: task3.cpp
Purpose: This code file processes images by applying adaptive thresholding, 
cleaning up noise, and creating a region map to highlight distinct areas based on size. 
The images are read from an input directory, processed to generate colored maps of selected regions, 
displayed, and saved to an output directory.
*/
...
// Removes small noise from the binary image using morphological operations.
...
// Generates a color-coded map of connected regions based on size constraints.
...
// Processes each image in the directory by thresholding, cleaning, and mapping regions.
...
// Validates command-line arguments and initiates processing if input is a directory.
